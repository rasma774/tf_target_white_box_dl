...
def read_GS():
    GS = pd.read_csv('human.source', sep='\t', header=None).iloc[:,[0,2]]
    return GS
...
def main():
    ...
    for fname in files:
        ...
        all_res[fname] = vals
        all_res_p[fname] = p
        with open('results_regnetwork_individual.txt', 'a') as f:
            f.write(fname)
            f.write('\nMedian enrichment: ' + str(np.nanmedian(vals)))
            f.write('\n\n')
    pd.DataFrame(all_res).to_csv('regnetwork_individual_lightup_results.csv')
    pd.DataFrame(all_res_p).to_csv('regnetwork_individual_lightup_results_prob.csv')
main()
